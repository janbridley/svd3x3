...
inline void approxGivensAngle(
    double a11, double a12, double a22, double& ch, double& sh) {
    // if ((a22 - a11) < EPSILON) {
    //   ch = sh = sqrt(2);
    //   return;
    // }
...
inline void QRDecomposition(double b[3][3], double q[3][3], double r[3][3]) {
    ...
    double sh22 = sh2 * sh2;
    double sh32 = sh3 * sh3;
    // TODO: is it faster if we write out the multiplications?
